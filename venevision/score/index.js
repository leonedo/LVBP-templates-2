...
webcg.on("data", function (data) {
  ...
  for (const key of Object.keys(data)) {
    if (key.includes("Color")) {
      ...
      delete data[key];
    } else if (key.includes("out") || key.includes("basellena")) {
    ...
    .then((resolve) => {
      ...
      setTimeout(() => {
        for (let i = 0; i < animElementsLength; i++) {
          ...
          if (
            animElement.hasOwnProperty("data") &&
            animElement.data.hasOwnProperty("cl") &&
            data &&
            data.hasOwnProperty(animElement.data.cl)
          ) {
            let cl = animElement.data.cl;
            let newPath;

            if (
              animElement.data.hasOwnProperty("refId") &&
              animElement.data.refId.includes("image")
            ) {
